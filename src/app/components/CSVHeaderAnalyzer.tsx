...
interface CSVHeaderAnalyzerProps {
  onStructureSelect: (structureName: string, file: File) => void;
...
}: CSVHeaderAnalyzerProps) => {
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [results, setResults] = useState<StructureMatch[] | null>(null);
      ...
      try {
        ...
        if (structureShortName) {
          if (!potentialShortname) {
            setShortnameError("Expected structure shortname in first row");
        ...
        const searchPromises = parsedHeaders.map(async (header) => {
          try {
            const normalizedHeader = header.toLowerCase().replace(/[_-]/g, "");

            let response = await fetch(`${NDA_DATA_ELEMENT}/${header}`);
            ...
            console.log(`Error fetching matches for field: ${header}`, err);
            return [] as string[];
          }
        });
        ...
        const structureCounts: Record<
          string,
          { name: string; matchingFields: string[]; matchCount: number }
        > = {};
        ...
        const sortedResults: StructureMatch[] = Object.values(structureCounts)
          .sort((a, b) => b.matchCount - a.matchCount)
          .map((result) => ({
            ...result,
            matchPercentage: (result.matchCount / parsedHeaders.length) * 100,
          }));
      ...
      {shortnameError && (
        <div className="bg-red-50 p-4 rounded-lg flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-red-600 text-sm mt-1">{shortnameError}</p>
          </div>
        </div>
      )}
            ...
            <div className="bg-blue-50 p-4 rounded-lg flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-blue-600 text-sm mt-1">
                  ...
                  <span className="font-mono">{detectedShortname}</span>
                </p>
              </div>
            </div>
          )}
              ...
              {results.length > 0 ? (
                results.map((result) => (
                  <div
                    ...
                    onClick={() => onStructureSelect(result.name, currentFile!)}
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <span className="text-sm font-medium">
                          {result.matchCount} of {headers.length} fields
                        </span>
                        <div className="text-xs text-gray-500">
                          {result.matchPercentage.toFixed(1)}% match
                        </div>
                      </div>
                    </div>
