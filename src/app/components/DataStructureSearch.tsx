...
// Re-use the same ValidatorState shape that CSVValidator accepts
type ValidatorState = React.ComponentProps<typeof CSVValidator>["validatorState"];
...
interface DataStructureSearchProps {
  searchTerm: string;
  ...
  handleStructureSelect: (structure: DataStructure) => void;
  dataElements: DataElement[];
  loadingElements: boolean;
  handleStructureSearch: (query: string) => void;
  ...
  databaseElements: Map<string, DataElement> | null | undefined;
  databaseConnectionError: string | null;
  onSwitchToElementSearch: (elementName: string) => void;
}
...
const DataStructureSearch = ({
  ...
  >({});
  const [availableCategories, setAvailableCategories] = useState<Set<string>>(
    new Set()
  );
  const [dataStructuresMap, setDataStructuresMap] = useState<
    Record<string, DatabaseStructureEntry>
  >({});
  ...
  useEffect(() => {
    const checkIfCustomTag = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const allTags = (await response.json()) as CustomTag[];
          const customTag = allTags.find((tag) => {
            if (isCategory) {
              ...
              return tag.tagType === "Data Type" && tag.name === filterValue;
            }
          });
    ...
    const fetchCategoryInfo = async () => {
      try {
        ...
        if (tagsResponse.ok) {
          ...
          const removedCategoriesMap: Record<string, Set<string>> = {};
          ...
          for (const tag of removedCategoryTags) {
            ...
            if (parts.length >= 3) {
              ...
              if (!removedCategoriesMap[structureShortName]) {
                removedCategoriesMap[structureShortName] = new Set();
              }
              removedCategoriesMap[structureShortName].add(categoryName);
            }
          }
  ...
  // Fetch database structures map
  useEffect(() => {
    const fetchDataStructures = async () => {
      try {
        const response = await fetch("/api/v1/data-structures");
        const data = (await response.json()) as {
          dataStructures?: DatabaseStructureEntry[] | Record<string, DatabaseStructureEntry>;
        };

        if (data?.dataStructures) {
          const map: Record<string, DatabaseStructureEntry> = {};
          const structureList = Array.isArray(data.dataStructures)
            ? data.dataStructures
            : Object.values(data.dataStructures);

          structureList.forEach((structure) => {
            const key =
              (structure.shortName as string | undefined)?.toLowerCase() ||
              (structure.name as string | undefined)?.toLowerCase();
            if (key) {
              map[key] = structure;
              if (structure.shortName) {
                map[structure.shortName as string] = structure;
              }
            }
          });
  ...
  useEffect(() => {
    ...
    const fetchTagsForStructure = async () => {
      try {
          ...
          try {
            ...
            if (dsResponse.ok) {
              const dsData = (await dsResponse.json()) as {
                dataStructures?: Array<{ shortName: string }>;
              };
              return { tag, dataStructures: dsData.dataStructures || [] };
            }
            return { tag, dataStructures: [] as Array<{ shortName: string }> };
          } catch (err) {
        ...
        const categoryTags: CustomTag[] = [];
        const dataTypeTags: CustomTag[] = [];
        const structureShortNameLower = selectedStructure.shortName.toLowerCase();

        tagStructures.forEach(({ tag, dataStructures }) => {
          const hasStructure = dataStructures.some(
            (ds) => ds.shortName?.toLowerCase() === structureShortNameLower
          );
  ...
  const handleDataTypeClick = async (dataType: string | undefined) => {
    if (!dataType) return;
    setIsExpanded(false);
  ...
  const handleElementDoubleClick = (elementName: string) => {
      ...
      <div className="flex flex-col h-screen">
        ...
        <div className="flex-none">
          <div className="mb-8">
            ...
            <p className="text-gray-600 -mb-7">Search the NDA Data Dictionary</p>
                ...
                <div className="bg-white rounded-lg shadow h-full overflow-y-auto">
                  <div
                    className={`sticky top-0 transition-transform duration-300 bg-white z-10 ${
                      showResultsHeader ? "translate-y-0" : "-translate-y-full"
                    }`}
                  >
                    <div className="p-4">
                      ...
                      <div className="mt-2 flex items-center gap-2 flex-wrap">
                        {searchTerm.startsWith("category:") && (
                          <span className="px-2 py-1 rounded-full text-xs inline-block bg-blue-100 text-blue-700">
                        ...
                        {searchTerm.startsWith("datatype:") && (
                          <span className="px-2 py-1 rounded-full text-xs inline-block bg-gray-100 text-gray-700">
                            {isCurrentFilterCustomTag && (
                              <span className="mr-1 text-orange-500">★</span>
                            )}
                            {searchTerm.replace("datatype:", "")}
                    ...
                    {structures.map((structure) => (
                      <div
                        ...
                        className={`p-4 border rounded hover:bg-gray-50 cursor-pointer transition-colors ${
                          selectedStructure?.shortName === structure.shortName
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-sm bg-gray-100 px-2 py-1 rounded">
                            {(structure as DataStructure & { source?: string }).source || "NDA"}
                          </span>
                        </div>
                        <p className="text-base mt-2">{structure.title}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  {databaseFilterEnabled && databaseStructures.length > 0 ? (
                    <>
                      ...
                      <p className="text-gray-600 mb-4">
                        No structures in your database match &quot;{searchTerm}
                        &quot;.
                      </p>
                      <button
                        onClick={() => setDatabaseFilterEnabled(false)}
                    ...
                    <>
                      ...
                      <p className="text-gray-600">
                        No structures match &quot;{searchTerm}&quot;.
                      </p>
                    </>
                  )}
                </div>
              ...
              >
                <div className="h-full">
                  {isExpanded && (
                    <div
                      className={`sticky top-0 z-10 bg-white shadow-sm transition-transform duration-300 ${
                        showDetailsHeader ? "translate-y-0" : "-translate-y-full"
                      }`}
                    >
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (selectedStructure) setIsExpanded(true);
                    }}
                  >
                      ...
                      <div className="bg-white p-6 rounded-lg shadow">
                        <div className="mb-8">
                          ...
                          {(() => {
                            ...
                            const projects =
                              (dbStructure?.submittedByProjects as string[] | undefined) || [];
                              ...
                              return (
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {projects.map((project, idx) => (
                                    <span
                                      key={idx}
                                      className="text-xs px-2 py-1 bg-indigo-100 text-gray-700 rounded"
                                    >
                                      {project}
                                    </span>
                                  ))}
                          ...
                          <div className="space-y-8">
                            <div className="grid grid-cols-3 gap-x-24 gap-y-6">
                                  ...
                                  <div className="text-lg font-mono font-medium text-blue-600 flex items-center">
                                    ...
                                    {databaseStructures
                                      ...
                                      <div className="relative group ml-2">
                                        ...
                                        <div className="absolute bottom-full left-0 mb-2 px-2 py-1 text-xs text-white bg-gray-800 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10">
                                          This structure exists in the IMPACT-MH
                                          database
                                        </div>
                                      </div>
                                    )}
                              ...
                              <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {(() => {
                                    ...
                                    const originalCategoryNames = new Set(
                                      originalCategories
                                    );
                                    ...
                                    const uniqueCustomTags =
                                      customCategoryTags.filter(
                                        (tag) =>
                                          !originalCategoryNames.has(tag.name)
                                      );

                                    const isTrulyCustomTag = (
                                      tagName: string
                                    ) => !availableCategories.has(tagName);
                                    ...
                                    const allCategories = [
                                      ...
                                      ...uniqueCustomTags.map((tag) => ({
                                        name: tag.name,
                                        isCustom: true,
                                        isTrulyCustom: isTrulyCustomTag(
                                          tag.name
                                        ),
                                        id: tag.id,
                                      })),
                                    ...
                                    return allCategories.map((item) => (
                                      ...
                                      >
                                        {item.name}
                                        {item.isCustom && item.isTrulyCustom && (
                                          <span className="ml-1 text-xs text-orange-500">
                                            ★
                                          </span>
                                        )}
                              ...
                              <div>
                                ...
                                <button
                                  onClick={() =>
                                    setIsCsvValidatorOpen(!isCsvValidatorOpen)
                                  }
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                  ...
                                  <tbody className="bg-white divide-y divide-gray-200">
                                    {dataElements.map((element, index) => {
                                      const isIncluded = headers.includes(
                                        element.name
                                      );
                                      const isInDatabase =
                                            ...
                                            <div className="flex items-center space-x-2">
                                              {isIncluded && (
                                                <CheckCircle
                                                  ...
                                                  aria-label="Included in uploaded CSV"
                                                />
                                              )}
                                              {isInDatabase && (
                                                <div className="relative group">
